...
describe('M1-0 addendum owner cases — verified GameSession boundary', () => {
  ...
  it('GE-M1-SES-002 rejects PLAYER create and seat assignment with zero state, event or version mutation', () => {
    ...
    expect(testHarness.store.snapshot(GAME_ID)).toBeUndefined();
  ...
  it('GE-M1-SES-003 materializes authenticated P1 membership and an active GameParticipant without payload authority', () => {
    ...
    const afterP1 = testHarness.store.snapshot(GAME_ID);
    const p2Join = testHarness.app.execute(sessionId('P2'), command('JOIN_GAME_MEMBERSHIP', GAME_ID, afterP1?.version ?? -1, {}));
    const state = testHarness.store.snapshot(GAME_ID);
    ...
    const rivalProjection = testHarness.app.getGameProjection(sessionId('P2'), GAME_ID);
    ...
    expect(p2Join.status).toBe('RESOLVED');
    expect(p1Projection.ok && p1Projection.projection.viewer).toEqual({ participantId: 'P1', role: 'PLAYER' });
    expect(rivalProjection.ok && rivalProjection.projection.participants.find(({ participantId }) => participantId === 'P1')).toEqual({
      participantId: 'P1',
      role: 'PLAYER',
      strategySubmitted: false,
      strategyLocked: false,
      handSize: 0,
      operationsDeckRemainingCount: 0,
    });
    expect(JSON.stringify(rivalProjection)).not.toMatch(/cardInstanceId|submittedCardInstanceIds|operationsDeckOrder|handCardInstanceIds/);
    expect(f1Projection.ok && f1Projection.projection.participants.map(({ participantId }) => participantId)).toEqual(['F1', 'P1', 'P2']);
  });
  ...
  it('GE-M1-SES-005 rejects a second seat for P1 atomically', () => {
    ...
    const beforeSecond = testHarness.store.snapshot(GAME_ID);
    ...
    expect(testHarness.store.snapshot(GAME_ID)).toEqual(beforeSecond);
  });
  ...
  it('GE-M1-SES-010 returns the same create and seat results on exact retries without duplication', () => {
    ...
    expect(testHarness.store.snapshot(GAME_ID)?.events.filter(({ type }) => type === 'GAME_CREATED')).toHaveLength(1);
    ...
    const afterSeat = testHarness.store.snapshot(GAME_ID);
    const seatRetry = testHarness.app.execute(sessionId('F1'), seatInput);

    expect(seatRetry).toEqual(seated);
    expect(testHarness.store.snapshot(GAME_ID)).toEqual(afterSeat);
