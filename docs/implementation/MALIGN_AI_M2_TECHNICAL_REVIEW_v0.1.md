...
**Corrección en progreso:** `M2_EFFECT_MANIFEST` materializa y prueba por igualdad exacta los 59 pares `effect_id`/`source_definition_id` del snapshot DEC-077. El dispatcher distingue ID desconocido (`EFFECT_UNKNOWN`) de ID aprobado sin handler (`EFFECT_DISABLED`), siempre sin mutación. Se implementaron los 23 bindings `CV_PAIR_BONUS` con validación de su pareja exacta, seis `TARGET_DT_SET` ligados a campaña/DT del escenario, tres efectos fijos de recursos/lifecycle y E017 con pago y descarte atómicos; sumados a los handlers previos, el dispatcher tiene 40 IDs ejecutables contando la habilidad de régimen. Tres efectos registrados se ejecutan por eventos: E033 devuelve D061 a la mano de su propietario impreso desde una campaña; E040 exige una campaña disparadora válida, usa RNG interno y la descarta con roll exitoso; E054 exige la instancia D026 recién robada y descarta ambas cartas. Permanecen 17 IDs del manifest sin implementación completa entre dispatcher, lifecycle y Reaction. Los handlers parciales restantes no se contabilizan hasta cerrar sus interacciones. M2R-R04 continúa **OPEN** y no se presenta el inventario como cobertura funcional 59/59.
