...
**Checkpoint de costes de componentes:** Ejercicios Militares y Movilización Militar se incorporaron al registry ejecutable de campaña con sus IV aprobados y cobran por activación el coste base del tier más `+1`/`+3`, con entradas separadas `CAMPAIGN_ACTIVATION_COST` y `CAMPAIGN_COMPONENT_COST`, total auditable en evento/trace y rechazo pre-roll atómico cuando falta el total. Tres regresiones integradas cubren campañas válidas MEDIUM y fondos insuficientes. `IQ-M2-018` registra una contradicción real: `GE-ERT-009/010` exigen una campaña LOW imposible con los IV 6/6 aprobados, por lo que esos dos owners no se declaran satisfechos ni se altera el oracle hasta resolución normativa.
