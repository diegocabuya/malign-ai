...
**Corrección en progreso:** se añadió un seam bidireccional tipado `buildM2StateFromCanonical` / `applyM2StateToCanonical`, con regresión `M2R-R01.integrated-state`, para que Resources, VP, cards/campaigns, influence, legitimacy y scheduler utilicen el `SetupGameState` canónico. Cleanup, End Game, Reaction y los handlers ejecutables cruzan puertos internos y `PostgresGameSessionApplication.coordinateDurableOperation`; `PASS_REACTION`/`PLAY_REACTION` cruzan además la sesión autenticada. `InternalM2CoreOperationPort` integra backlash, legitimidad, modificación/descarte de campaña, Starter y robo ciego con ownership, phase/CAS/idempotencia, RNG transaccional, lifecycle y auditoría. El estado canónico conserva ahora `returnToOwnerOnDiscard`, `REMOVED_FROM_GAME` y sincroniza hands/discards al transferir control. AuthorizedProjection limita las opciones de Reaction al actor prioritario y a F1. Ninguna frontera acepta ActorContext, permisos, métricas o rolls libres del caller. M2R-R01 permanece **OPEN** hasta integrar scheduler/resume y las operaciones restantes; los handlers ausentes se rastrean en M2R-R04.
