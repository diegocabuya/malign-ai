...
**Corrección en progreso:** `M2_EFFECT_MANIFEST` materializa y prueba por igualdad exacta los 59 pares `effect_id`/`source_definition_id` del snapshot DEC-077. El dispatcher distingue ID desconocido (`EFFECT_UNKNOWN`) de ID aprobado sin handler (`EFFECT_DISABLED`), siempre sin mutación. Se implementaron los 23 bindings `CV_PAIR_BONUS` con validación de su pareja exacta, seis `TARGET_DT_SET` ligados a campaña/DT del escenario, tres efectos fijos de recursos/lifecycle y E017 con pago y descarte atómicos; sumados a los handlers previos, el dispatcher tiene 40 IDs ejecutables contando la habilidad de régimen. Siete efectos registrados se ejecutan por eventos: E033 cubre `CAMPAIGN_TO_DISCARD`; E010, E012, E022 y E036 exigen la carta disparadora exacta (D043, D012, D041 y D088) y E022 conserva D043 en su ventana hija; E040 exige campaña disparadora y RNG interno; E054 exige la instancia D026 recién robada y descarta ambas cartas. Permanecen 13 IDs del manifest sin implementación completa entre dispatcher, lifecycle y Reaction. Los handlers parciales restantes no se contabilizan hasta cerrar sus interacciones. M2R-R04 continúa **OPEN** y no se presenta el inventario como cobertura funcional 59/59.
