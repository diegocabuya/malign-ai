...
**Corrección en progreso:** se añadió un seam bidireccional tipado `buildM2StateFromCanonical` / `applyM2StateToCanonical`, con regresión `M2R-R01.integrated-state`, para que Resources, VP, cards/campaigns, influence, legitimacy y scheduler utilicen el `SetupGameState` canónico. `INTERNAL_RUN_M2_CLEANUP` e `INTERNAL_RUN_M2_END_GAME` ya atraviesan `dispatchAtomicCommand` con phase/CAS/idempotencia/eventos; sus puertos internos los ejecutan mediante `PostgresGameSessionApplication.coordinateDurableOperation`, sin ActorContext ni métricas de puntuación suministrados por caller. End Game construye las métricas desde demographics e influence canónicos y persiste los VP finales y el outcome en la misma transición. M2R-R01 permanece **OPEN** hasta conectar Reaction por la misma frontera.
