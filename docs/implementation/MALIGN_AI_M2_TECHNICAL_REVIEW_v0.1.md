...
**Corrección en progreso:** `GE-ACT-002…005`, `GE-ACT-018…023` y `GE-ACT-027` ya dejaron de ejecutarse en el loop superficial y conservan sus IDs en escenarios canónicos autenticados. Estos escenarios atraviesan dispatcher, CAS, continuaciones privadas, RNG autoritativo, lifecycle, hand limit, recursos y auditoría observable. La sustitución encontró y corrigió tres desviaciones ocultas: Double Agent exige `min(5, hand)` en vez de cinco cartas inexistentes; Discard Action permite cero recuperaciones elegibles; Cyber Theft registra todos los D10 y transfiere sólo para `<=4`, tratando un target sin recursos como transferencia cero en vez de rechazar toda la acción. Al eliminar ocho duplicados superficiales ya sustituidos, la suite no-PostgreSQL vigente reporta **501/501 PASS en 43 archivos**; la reducción es eliminación de evidencia inválida, no pérdida de IDs owner. M2R-R03 permanece **OPEN** hasta reemplazar los owners superficiales restantes de M2-3/M2-4/M2-6.
