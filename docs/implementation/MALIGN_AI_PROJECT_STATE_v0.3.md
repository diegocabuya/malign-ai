...
**Fecha:** 2026-08-28
**Fase actual:** M1 CLOSED — M2-0 APPROVED AND CLOSED — M2-A/M2-1 CORRECTION M2A-R20…R24 IMPLEMENTED / PENDING REVIEW mediante DEC-078/DEC-079 — M2-2…M2-7, M2 global y M3 NOT AUTHORIZED
...
| M2 Implementation Spec v0.1 | **M2-A/M2-1 CORRECTION M2A-R20…R24 IMPLEMENTED / PENDING REVIEW mediante DEC-078/DEC-079** |
| M2-A correction gate | **22/22 owner preservados + 14/14 regresiones asignadas preservadas + 6/6 regresiones M2A-R20…R24 PASS; suite M2-A 31/31 PASS** |
...
| Suite acumulada tras la corrección M2-A | **246/246 PASS en 28 archivos, 0 skips, 0 todo, 0 waivers** |
...
| M2-A/M2-1 — PostgreSQL Persistence and Durable Recovery | **CORRECTION M2A-R20…R24 IMPLEMENTED / PENDING REVIEW mediante DEC-078/DEC-079** |
...
M1-0 está formalmente cerrado mediante DEC-067, M1-1 mediante DEC-069, M1-2 mediante DEC-071 y M1-3 mediante DEC-073. M1 global está **IMPLEMENTED AND APPROVED / CLOSED**. El planning gate M2 queda **APPROVED AND CLOSED mediante DEC-076**, M2-0 queda **APPROVED AND CLOSED mediante DEC-077**, y M2-A/M2-1 queda **CORRECTION M2A-R20…R24 IMPLEMENTED / PENDING REVIEW mediante DEC-078/DEC-079**. M2-2…M2-7, M2 global y M3 permanecen **NOT AUTHORIZED**.
...
M2-A implementa PostgreSQL **18.6** con driver `pg` **8.23.0**, seis migrations SQL forward-only, ledger técnico separado y manifest contractual independiente y explícito de **87 tablas, 865 columnas, 1260 constraints, 210 índices, 25 triggers y 5 functions**, contrastado contra la Physical Database Spec blob `13cd601b30db2db22be64c4fda5df94144dcf8d5`, con catalog SHA-256 `da6388288fb273409fc52ed80645186e7f58c21a17af1351a82c7814e6c117a4`. Las migrations `001…005` permanecen intactas; `005_contractual_integrity_and_privileges.sql` conserva SHA-256 `6d60164092fd8c72e8e2d3d3b2df988481a8017b7473ba05c3948642b95c1580`; la nueva `006_durable_parity_and_least_privilege.sql` tiene SHA-256 `1c46e60136fa2967714d52186e89af3564fca342b11269cf3c572e54914aa317`. El registry aprobado valida JCS SHA-256 `735fd01b65416bdeb1baaa596bb36ea0d0eef31cb1d1d9b7f4b2322c9c585e4a`, siembra 100 definitions, 108 templates, 4 aliases, 59 effects y 103 operaciones, y materializa por Game 540 CardInstance —108 por país— incluidas 25 Starter.
...
El adapter PostgreSQL satisface el port M1 completo para lifecycle, setup, initiative, maintenance, action planning, interacción de adjudicación, proyecciones y initial sync/feed de aplicación, sin transporte productivo. El Engine produce una transición durable DB-free, tipada y versionada con before/after, eventos, traces, mutaciones por familia, ledgers, continuation, actoría y causalidad; el completeness guard rechaza antes de todo write cualquier transición incompleta o adulterada. El Unit of Work usa `READ COMMITTED`, fast lookup de idempotencia, `SELECT ... FOR UPDATE`, recheck bajo lock, CAS de `game_version`, un único `pg` client, persistencia normalizada/journals/event/trace/continuation/idempotencia/outbox en una transacción y publicación sólo post-commit.

Recovery y reconciliation contrastan snapshot+tail y `authoritative_state_json` contra phase/initiative, AP/Resources/VP, cartas/zonas/mazo, plans, campaigns, PD/influence/legitimacy, choices/continuations y heads/pins normalizados. Una divergencia fija `recovery_blocked` y añade un trace diagnóstico F1-only con actoría `SYSTEM` y `participant_id=NULL`, sin gameplay event ni incremento de versión. Los roles de producto continúan `NOLOGIN`; los pools de prueba usan principals `LOGIN` efímeros, no administrativos, con exactamente una membership correspondiente, y UoW/recovery/publisher fallan cerrado ante una identidad incompatible. OutboxMessage es inmutable, DeliveryState mutable y DeliveryAttempt append-only; no se afirma exactly-once delivery.

El owner gate M2-A conserva **22/22 PASS**, las **14/14 regresiones asignadas previas** permanecen verdes y las seis regresiones ejecutables M2A-R20…R24 reportan **6/6 PASS**: adapter PostgreSQL completo, completeness guard, adjudicación M1 real y reconciliación, divergencia normalizada, manifest independiente y principals/ACL reales. El gate M2-A suma **31/31 PASS**; baseline M0/M1 **215/215 preservada**; suite acumulada **246/246 PASS en 28 archivos, 0 skips, 0 todo y 0 waivers**. La corrección funcional publicada está fijada en `bc186765da642363fb3fb1a73f217f3cbd19b1bd`. DEC-078/DEC-079 no constituyen cierre final: M2-A/M2-1 permanece **CORRECTION M2A-R20…R24 IMPLEMENTED / PENDING REVIEW**. IQ-M2-014 e IQ-M2-015 quedan resueltas; no apareció una nueva `IMPLEMENTATION_QUESTION`. IQ-M2-008/009 permanecen OPEN y sólo bloquean M2-2.
