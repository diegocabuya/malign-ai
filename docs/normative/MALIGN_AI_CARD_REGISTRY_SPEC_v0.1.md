...
**Fecha:** 2026-08-26
**Estado:** **APPROVED / SEEDABLE mediante DEC-077 — M2-0 CLOSED**
...
|---|---:|---|
| `CardDefinition` lógicas | 100 | IDs aprobados mediante DEC-077 |
| `CountryCardSerialTemplate` por country set | 108 | mapping 108→100 aprobado mediante DEC-077 |
| Starter templates por country | 5 | estructura, identidad y contenido aprobados |
...
| Aliases | 4 | lookup-only aprobados por DEC-029 y DEC-077 |
| Effect definitions | 59 | IDs/trigger/timing/operations aprobados mediante DEC-077 |
| Definitions sin texto de efecto observado | 41 | ausencia primaria aprobada mediante DEC-077 |
| Operaciones tipadas | 103 | 103 con parámetros machine-readable completos y aprobados; 0 unknown/N/A |
| Auditoría primaria | 108/108 páginas | 102 MATCH, 6 DIFFERENCE, 0 AMBIGUOUS; aprobada mediante DEC-077 |

El snapshot aprobado tiene status exacto `approved`, `seedable=true`, 100 definitions ordenadas, 108 templates ordenados, 59 effects, 103 operaciones y 108 filas de auditoría primaria. DEC-077 conserva como evidencia el JCS candidato `eb98696020d3694acd8a3374d27ec064ef6db16fd6ea083bb4eaeaac9b30ba74` y aprueba el artifact final con JCS SHA-256 `735fd01b65416bdeb1baaa596bb36ea0d0eef31cb1d1d9b7f4b2322c9c585e4a` y Git blob SHA `8d5c150bed742391555bc6bafe022f45baee0163`.
...
- parameters machine-readable con `operation_ordinal`, valores operativos concretos, provenance, `fail_closed_if_unsupported=true` y estado `complete_approved_dec_077`.

Las 59 definitions se descomponen en 23 pair bonuses, 16 Action, 6 Reaction, 5 Starter y 9 campaign-special/target effects. La auditoría cubre **103/103 operaciones: 103 complete_approved_dec_077, 0 unknown, 0 N/A y 0 unresolved_parameter_id**. DEC-077 aprueba IDs, triggers, timings, orden y parámetros; una operación, versión, trigger o parámetro no soportado continúa fallando cerrado. Las operaciones en el JSON son datos declarativos, nunca JavaScript, SQL, prompt o nombre de función.
...
Los literales primarios de los seriales 26 y 28 permanecen intactos. Product Owner M20-R10 aprobó estos bindings internos: en serial 26, `DP` → `PD` — Population Demographic; en serial 28, `cubos de resistencia` → cubos azules `RESILIENCY` y `DP` → `PD`. DEC-077 aprueba REG-CAND-003 completo y conserva la auditoría en 102 MATCH, 6 DIFFERENCE y 0 AMBIGUOUS.
...
- DEC-077 fija JCS SHA-256, Git blob hashes, source digests y decisión de aprobación de esta versión.
...
|---|---|---|
| Snapshot candidato anterior | SHA-256 sobre bytes JCS | `eb98696020d3694acd8a3374d27ec064ef6db16fd6ea083bb4eaeaac9b30ba74` |
| Snapshot aprobado canonicalizado | SHA-256 sobre bytes JCS | `735fd01b65416bdeb1baaa596bb36ea0d0eef31cb1d1d9b7f4b2322c9c585e4a` |
| Snapshot aprobado pretty JSON | Git blob SHA-1 | `8d5c150bed742391555bc6bafe022f45baee0163` |
| Proyección semántica antes/después | SHA-256 JCS; 264610 bytes | `8a46133ca70883df2d173fddd9c725cd0611b2be8311a5fe42057464415d6a13` |
| Este Markdown | Git blob SHA-1 | se calcula después de cerrar el artifact y se registra en gate/state |

Estos hashes quedan aprobados mediante DEC-077; el snapshot no contiene su propio hash final para evitar autorreferencia.

## 12. Trazabilidad a aceptación

| Concern | Fuente/gate | Estado candidato |
|---|---|---|
| 108/país, 5 Starter, 103 pool, 540/25 | DEC-025; GE-SET-007/008; GE-E2E-006; GE-M2-DB-005 | aprobado mediante DEC-077 |
| versions/hash/seed | GE-AUD-003/004; GE-M2-DB-003/005/006; GE-M2-TX-008/009 | snapshot aprobado y seedable; seed no ejecutado |
...
| Action/Starter | GE-ACT-001…030; GE-PLAN-006…010 | 21 definitions/effects y mappings aprobados |
...
| primary source audit | M20-R09 / REG-CAND-001…003 | 108/108 páginas, 59 literales/41 ausencias, 102/6/0; aprobado mediante DEC-077 |

## 13. Aprobaciones de cierre

1. `REG-CAND-001`: **APPROVED** — 100 IDs, mapping 108→100 y seis grupos repetidos;
2. `REG-CAND-002`: **APPROVED** — contenido impreso, aliases, clasificación, IV, costes, flags, cinco Starter y 41 ausencias;
3. `REG-CAND-003`: **APPROVED** — 59 effect IDs, triggers, timings, 103 operaciones, orden, parámetros, E021 y bindings 26/28;
4. `REG-CAND-004`: **APPROVED** — JCS y Git blobs finales fijados mediante DEC-077.

DEC-077 resuelve `IQ-M2-010`, aprueba y cierra M2-0 y habilita `seedable=true` como estado documental. No ejecuta seeds ni autoriza M2-1/M2-A, M2-2…M2-7, M2 global o M3.

## 14. Estado del gate

M20-R01…R10 quedan **CLOSED** mediante DEC-077. Registry Spec y Snapshot quedan **APPROVED**; el snapshot queda **SEEDABLE** y M2-0 **APPROVED AND CLOSED**. La proyección semántica antes/después permanece byte-for-byte idéntica. No se creó seed, fixture, handler ni código ejecutable; M2-1/M2-A, M2-2…M2-7, M2 global y M3 permanecen **NOT AUTHORIZED**.
