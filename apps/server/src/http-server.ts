...
const setupCommandTypes = new Set<SetupCommandType>([
  ...
  'PASS_REACTION', 'PLAY_REACTION', 'SUBMIT_VETO_DEFENSE', 'CAST_VETO_VOTE', 'SUBMIT_M2_EFFECT_CHOICE', 'SUBMIT_VIRAL_CHOICE',
  'RESOLVE_VETO_ABUSE', 'ACKNOWLEDGE_TEMPORARY_REVEAL',
]);
...
export const createAuthoritativeHttpServer = (options: AuthoritativeHttpServerOptions): Server => {
  ...
  const handleRequest = async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      const membership = await options.memberships.resolveMembership(session, input.gameId);
      if (input.commandType === 'SUBMIT_CHOICE' || input.commandType === 'SUBMIT_CAMPAIGN_NARRATIVE' || input.commandType === 'SUBMIT_COALITION_CONTRIBUTION' || input.commandType === 'SUBMIT_MANUAL_DIE') {
